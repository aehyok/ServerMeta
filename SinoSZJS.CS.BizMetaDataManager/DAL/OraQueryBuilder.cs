...
namespace SinoSZJS.CS.BizMetaDataManager.DAL
{
    ...
    public class OraQueryBuilder
    {
        ...
        public static Dictionary<string, string> GetQueryStr(MDModel_QueryModel _qv, MDQuery_Request _queryRequest, ref string _mainQueryStr)
        {
            ...
            else
            {
                ...
                _mainQueryStr = string.Format("select {0} from {1} where {4} ( {2} ({3})) ",
                        _displayStr, _tableStr, _tableRelationStr, _conditionStr, _filterStr);
                Dictionary<string, string> _ret = new Dictionary<string, string>();
                _ret.Add(_qv.MainTable.TableName, CreateMainTableResult(_qv, _queryRequest.MainResultTable));
                if (_queryRequest.ChildResultTables != null)
                {
                    foreach (MDQuery_ResultTable _rt in _queryRequest.ChildResultTables)
                    {
                        _ret.Add(_rt.TableName, CreateChildTableResult(_qv, _rt));
                    }
                }
                return _ret;
        ...
        /// <returns></returns>
        private static string CreateChildTableResult(MDModel_QueryModel _qv, MDQuery_ResultTable _rt)
        {
        ...
        /// <param name="_qv"></param>
        /// <returns></returns>
        public static string CreateConditionStringByMainID(MDModel_QueryModel _qv)
        {
            string _str = string.Format("{0}.{1} in (select  pk_c from query_temp) ", _qv.MainTable.TableName, _qv.MainTable.TableDefine.Table.MainKey);
            return _str;
        }
        ...
        public static string ReplaceExtSecretNoUserInfo(string _secret)
        {
            if (_secret == null) return _secret;
            string _retstr = _secret;
            _retstr = _retstr.Replace("%ROOTDWID%", ConfigFile.SytemDWRootID);
            ...
            else
            {
                switch (_tc.ColumnDataType)
                {
                    case "NUMBER":
                        ...
                        break;
                    case "DATE":
                        _sql.Append(string.Format(" to_date('{0}','yyyymmdd') ", _data));
                        break;
                    default:
                        _sql.Append(string.Format(" '{0}' ", _data));
                        break;
        ...
        public static string RebuildGuideLineQueryStringByDefault(string _queryStr, MDQuery_GuideLineParameter _gp)
        {
            ...
            switch (_gp.Paramter.ParameterType)
            {
                ...
                case "日期型":
                    ...
                    if (_s1.Length == 8)
                    {
                        _s1 = string.Format(" to_date('{0}','YYYYMMDD') ", _s1);
                    }
        ...
        public static string CreateDisplayMainKeyString(MDModel_QueryModel _qv)
        {
            string _res = string.Format("{0}.{1} MainID", _qv.MainTable.TableName, _qv.MainTable.TableDefine.Table.MainKey);
            return _res;
        }
